...
After checking the qc report you can select bad samples to remove. I would remove all outliers in `qc.summary$bad.samples` except the outliers based on control probes as there are often a few probes for each control type.  For the control probe categories, I would remove slides with dye bias `Control probe (dye.bias)` which are captured by the normalisation probes `(normC + normG)/(normA + normT)` or bisulfite conversion control probes (`Control probe (bisulfite1)` and `Control probe (bisulfite2)`) .
